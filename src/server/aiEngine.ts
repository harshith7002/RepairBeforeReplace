...
{
  "name": string (short product name, e.g. "Front-Loading Washing Machine" or "Mountain Bicycle Drivetrain"),
  ...
  "repairability": {
    ...
    "partsNote": string (short note on replacement component availability for this specific object),
    "complexityNote": string (short note on tools and skill level for this specific object),
    "costRatioNote": string (short note on repair vs replace cost ratio for this specific object),
    "accessibilityNote": string (short note on physical access to component)
  },
  ...
  "repairCostRange": string (INR range, e.g. "₹150 – ₹400"),
  "replaceCost": string (INR value, e.g. "₹22,000"),
  "replaceText": string (short description of what replacement entails for this specific object),
  "replacementDisposalNote": string (short note on waste generated by discarding this specific object),
  "estimatedTime": string (e.g. "15 – 25 min"),
  "difficulty": one of "Easy","Moderate","Advanced","Professional Recommended",
  "toolsRequired": [ { "name": string, "spec": string } ] (2-5 items),
  "safetyWarnings": string[] (1-4 items relevant to this specific object),
  "safetyCautionType": string (short caution title specific to this object type, e.g. "Mechanical Pinch Point Caution"),
  "safetyCautionDesc": string (short advice on when to seek professional shop service for this object),
  ...
  "impact": { "materialSavedKg": number, "co2SavedKg": number, "eWasteDivertedPercent": number, "materialNote": string (short list of materials saved for this specific object) }
}

Ground every field in what is actually visible in the photo (object type, apparent damage, wear, context clues). Be specific and realistic, not generic. Do NOT mention appliances or washing machines if the object is a bicycle, tool, laptop, or furniture. Costs should be realistic in Indian Rupees for this kind of object and repair.
...
function normalizeAiResult(raw: Record<string, unknown>): GeneratedDiagnosis {
  ...
  const factors = {
    ...
    partsNote: asString(repairabilityRaw.partsNote, fallback.repairabilityBase.partsNote || ''),
    complexityNote: asString(repairabilityRaw.complexityNote, fallback.repairabilityBase.complexityNote || ''),
    costRatioNote: asString(repairabilityRaw.costRatioNote, fallback.repairabilityBase.costRatioNote || ''),
    accessibilityNote: asString(repairabilityRaw.accessibilityNote, fallback.repairabilityBase.accessibilityNote || ''),
  };
  ...
  const impact: ImpactMetrics = {
    ...
    materialNote: asString(impactRaw.materialNote, fallback.impactBase.materialNote || ''),
  };
  ...
  const replaceText = asString(raw.replaceText, fallback.replaceText || '');
  const replacementDisposalNote = asString(raw.replacementDisposalNote, fallback.replacementDisposalNote || '');
  const safetyCautionType = asString(raw.safetyCautionType, fallback.safetyCautionType || '');
  const safetyCautionDesc = asString(raw.safetyCautionDesc, fallback.safetyCautionDesc || '');
